# GuadaClaw AI Electron 版本部署指南

本文档提供 GuadaClaw AI Electron 桌面应用的完整构建、打包和分发指南。
...
```
release/
├── GuadaClaw-AI-Setup-1.0.0.exe          # Windows 安装程序
├── GuadaClaw-AI-1.0.0.dmg                # macOS 安装包
├── GuadaClaw-AI-1.0.0.AppImage           # Linux AppImage
└── ...
```
...
#### Windows
```
C:\Users\<用户名>\AppData\Roaming\GuadaClaw\
...
#### macOS
```
~/Library/Application Support/GuadaClaw/
├── ai_chat.db
├── vector_db.sqlite
├── uploads/
├── settings.json
...
```bash
# Windows PowerShell
Copy-Item "$env:APPDATA\GuadaClaw" "D:\Backup\GuadaClaw-Backup-$(Get-Date -Format 'yyyyMMdd')"

# macOS/Linux
cp -r ~/Library/Application\ Support/GuadaClaw ~/Desktop/GuadaClaw-Backup-$(date +%Y%m%d)
```
...
修改为自定义路径：
```typescript
const userDataPath = 'D:\\GuadaClaw-Data';  // Windows
// 或
...
1. 确保没有其他 GuadaClaw 实例在运行
2. 删除锁文件：
   ```bash
   # Windows
   Remove-Item "$env:APPDATA\GuadaClaw\ai_chat.db-shm"
   Remove-Item "$env:APPDATA\GuadaClaw\ai_chat.db-wal"
   
   # macOS/Linux
   rm ~/Library/Application\ Support/GuadaClaw/ai_chat.db-shm
   rm ~/Library/Application\ Support/GuadaClaw/ai_chat.db-wal
   ```
...
2. 清除缓存并重启：
   ```bash
   # 删除缓存目录
   rm -rf ~/Library/Application\ Support/GuadaClaw/Cache
   # 或
   rm -rf ~/.config/GuadaClaw/Cache
   ```
...
1. 检查上传目录权限：
   ```bash
   # Linux/macOS
   chmod -R 755 ~/Library/Application\ Support/GuadaClaw/uploads
   ```
...
```powershell
# 查看最新错误
Get-Content "$env:APPDATA\GuadaClaw\logs\error-*.log" -Tail 50
```

**macOS/Linux**：
```bash
# 实时查看日志
tail -f ~/Library/Application\ Support/GuadaClaw/logs/combined-*.log

# 查看错误日志
grep "ERROR" ~/Library/Application\ Support/GuadaClaw/logs/error-*.log | tail -50
```
...
1. 使用系统卸载程序卸载应用
2. 手动删除用户数据目录：
   - Windows: `%APPDATA%\GuadaClaw`
   - macOS: `~/Library/Application Support/GuadaClaw`
   - Linux: `~/.config/GuadaClaw`
